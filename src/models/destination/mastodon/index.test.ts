import { mastodonStrategies } from "./";
...
describe("Mastodon Strategies", () => {
  ...
  describe("static properties", () => {
    test.each([
      ["Mastodon Text", "mastodon", 0],
      ["Mastodon Link", "mastodon", 1],
      ["Mastodon Media", "mastodon", 2],
    ])("Json should have name %p and site %p for inded %d", (name, site, index) => {
        expect(mastodonStrategies[index].name).toBe(name);
        expect(mastodonStrategies[index].site).toBe(site);
      });
  })

  describe("shouldRun", () => {
    test.each([
      ["Mastodon", 0, isTextShareMock],
      ["Mastodon", 1, isLinkShareMock],
      ["Mastodon", 2, isMediaShareMock],
        ...
        const result = mastodonStrategies[index].shouldRun(post, destination)
  ...
  describe("run", () => {
    test.each([
      ...
      const destination = {
       ...
       mastodonStrategies[index].run(post, destination)
